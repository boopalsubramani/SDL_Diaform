...
//                 <View style={styles.patientDetailsSection}>
//                     <View style={styles.patientDetailsRow}>
//                         <Text style={styles.patientDetailsLabelName}>Name: {selectedPatientDetails?.PtName || selectedPatientDetails?.Pt_Name}</Text>
//                     </View>
//                     <View style={styles.patientDetailsRow}>
...
const FinalPaymentScreen = ({ navigation, route, showHeader = true }: any) => {
    ...
    const calculateTotal = ({ tests }: any) => {
        ...
        const subTotal = validTests.reduce((sum, test) => sum + (parseFloat(test.Amount) || 0), 0);
        const discount = validTests.reduce((sum, test) => sum + (parseFloat(test.Discount_Amount) || 0), 0);
        const vatAmount = validTests.reduce((sum, test) => sum + (parseFloat(test.T_VAT_Amount) || 0), 0);
        const patientAmount = validTests.reduce((sum, test) => sum + (parseFloat(test.Patient_Share) || 0), 0);
        const netAmount = subTotal - discount + vatAmount;
        const netPayable = netAmount - patientAmount;
