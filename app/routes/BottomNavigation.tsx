...
const BottomNavigation = () => {
  ...
  // 🔥 Ensure the initial route exists in `menuItems`
  const initialScreen = menuItems.find(item => item.Main_Menu_Code === 'BT')?.Menu_Desc 
    || menuItems[0]?.Menu_Desc 
    || "Bookings"; // Default to first available screen

  // 🛑 Prevent rendering if menuItems is empty (avoid crash)
